...
| **Platform Operator**   | مشغّل المنصة        | The Founder / Super User as a **platform-level identity** — provisions Businesses and invites initial Admins. NOT a Business User and NOT a `UserRole` value; Founder Side UI arrives in Spec B.   |
| **Invitation**          | دعوة                | FlowPilot domain concept (separate from Better Auth): a request to join one Business as `ADMIN` or `STAFF`. Lifecycle `PENDING → ACCEPTED / EXPIRED / REVOKED`. **Planned — not yet implemented.** |
| **Account Activation**  | تفعيل الحساب        | An invited user accepting the invitation and setting a password, moving from `INVITED` to `ACTIVE`. **Planned — not yet implemented.**                                                             |
| **Business Activation** | تفعيل المنشأة       | The Business becoming operational after its initial Admin completes onboarding. Business lifecycle: `PROVISIONED → ACTIVE → DEACTIVATED`. **Planned — not yet implemented.**                       |
